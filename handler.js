...
(function() {
  (function($, window, document, undefined_) {
    ...
    return $.fn.quicksearch = function(target, opt) {
      var cache, jq_results, options, rowcache, self, timeout, val;
      this.removeDiacritics = function(str) {
        var changes, i;
        changes = $.quicksearch.diacriticsRemovalMap;
        i = 0;
      ...
      this.go = function() {
        var i, len, noresults, numMatchedRows, query, val_empty;
        i = 0;
        len = 0;
        numMatchedRows = 0;
        noresults = true;
      ...
      this.cache = function(doRedraw) {
        ...
        t = (typeof options.selector === "string" ? jq_results.find(options.selector) : $(target).not(options.noResults));
        cache = t.map(function() {
          var temp;
          temp = self.strip_html(this.innerHTML);
          if (options.removeDiacritics) {
            return self.removeDiacritics(temp);
          } else {
            return temp;
          }
        });
      ...
      return this.each(function() {
        return $(this).on(options.bind, function() {
          val = $(this).val();
          return self.trigger();
        });
      });
  ...
  $("document").ready(function() {
    ...
    $(".tag").on('click', function() {
      var container, scrollTo, tagname, tagstohide;
      tagname = $(this).attr("class").split(" ")[1];
      $('.tag.' + tagname).toggleClass('highlighted');
      if (tagsarray.indexOf(tagname) === -1) {
        tagsarray.push(tagname);
        container = $("#events");
        scrollTo = $("#events");
        $("html,body").animate({
          scrollTop: scrollTo.offset().top + container.scrollTop()
        }, 900);
      } else {
    ...
    $(".all").on('click', function() {
      ...
      $('input.search').quicksearch('.comicpage');
      return tagsarray = [];
    });
    $('input.search').quicksearch('.tags');
    $('input.search').quicksearch('.comicpage');
    ...
    $(".alpha").on("click", function() {
      var alphabet, tagsstartwith;
      alphabet = $(this).attr("class").split(" ")[1];
      if (alphabet === "num") {
        tagsstartwith = $("#jscloud").find("a[class*=' 0'], a[class*=' 1'], a[class*=' 2'], a[class*=' 3'], a[class*=' 4'], a[class*=' 5'], a[class*=' 6'], a[class*=' 7'], a[class*=' 8'], a[class*=' 9']");
      } else {
        tagsstartwith = $("#jscloud").find("a[class*=' " + alphabet + "']");
      }
    ...
    $(document).scroll(function() {
      y = $(this).scrollTop();
      if (y > 200) {
        return $("#topscroll").slideDown(300);
      } else {
        return $("#topscroll").slideUp(300);
      }
    });
    return $("#topscroll").on("click", function() {
